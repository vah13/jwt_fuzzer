...
# header fuzzing class
class _Fuzz(object):
    def __init__(self, header_or_body, inject_point):
        self.fuzz_data = header_or_body
        self.inject_point = inject_point
    ...
    """
    point header or body
    """
    def build_mutation_parameter(self, point):
        mutated_payloads = []
        payloads = self.get_payloads()
        for payload in payloads:
            payload = payload.strip()
            s = json_replacer(self.fuzz_data, self.inject_point, payload.replace("\"", "\\\""))
            mutated_payloads.append(encodeToBase64(s))
        return mutated_payloads
